...
- **`~/.cgdb/cgdbrc`** config file support (all `:set`, `:highlight`, `:map`, `:imap` commands)
- **`CGDB_DIR`** environment variable support
...
Options:
  ...
  -r, --rcfile FILE         Configuration file (default: ~/.cgdb/cgdbrc)
...
| `Ctrl-u` | Half page up |
| `gg` | Go to top |
| `G` | Go to bottom (or line N with numeric prefix) |
...
tgdb reads `~/.cgdb/cgdbrc` (or `$CGDB_DIR/cgdbrc`) on startup.
Commands can also be typed interactively via `:`.
